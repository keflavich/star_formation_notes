...
% M = 1e33 g
% v = 1e9 cm/s = 1e4 km/s
% p = E/v/m = 1e9 cm/s = 1e4 km/s
% astropy version: 2*(1e51*u.erg / (1e9*u.cm/u.s)/u.M_sun).to(u.km/u.s)
% <Quantity 10058.28843174 km / s>
% except it should be 10^49 erg per Msun averaged over the IMF, so it goes back down to ~100 km/s
...
\begin{equation}
\left\langle \frac{p_{\rm SN}}{M}\right\rangle = \frac{2}{v_{\rm ej}} \left\langle \frac{E_{\rm SN}}{M}\right\rangle = 10^4 v_{\rm ej,9}^{-1} \mbox{ km s}^{-1}.
\end{equation}
Physically, this means that every $\msun$ of matter than goes into stars provides enough momentum to raise another $\msun$ of matter to a speed of 10$^2$ km s$^{-1}$. This is greater than other feedbacks already, which is why SNe have commonly been assumed to be the dominant source of feedback on galactic scales.  Additionally, supernovae, like stellar winds, may have an energy-conserving phase where their momentum deposition grows. We will discuss the question of supernova momentum deposition more in Chapter \ref{ch:sflaw_th} in the context of models for regulation of the star formation rate.
